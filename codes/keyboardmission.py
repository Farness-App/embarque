...
def fonction_haut(event=None):
    ...
    success = drone(moveBy(0, 0, -0.1, 0)).wait().success()
    if not success:
...
def fonction_avance(event=None):
    ...
    success = drone(moveBy(0.1, 0, 0, 0)).wait().success()
    if not success:
        print("Erreur de déplacement !")
...
def fonction_gauche(event=None):
    ...
    rotation_radians = math.radians(-5)
    success = drone(moveBy(0, 0, 0, rotation_radians)).wait().success()
...
def fonction_atterrissage(event=None):
    ...
    drone.disconnect()
    fenetre.quit()  # Fermer l'interface graphique après atterrissage
...
btn_urgent = tk.Button(fenetre, text="urgent", command=fonction_retour_maison)
btn_atterrissage = tk.Button(fenetre, text="Atter", command=fonction_atterrissage)

btn_urgent.grid(row=0,column=0)
...
fenetre.bind("e", fonction_retour_maison)
