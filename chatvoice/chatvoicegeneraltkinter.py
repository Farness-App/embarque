...
# Initialisation de l'état du drone
...
fenetre = tk.Tk()
fenetre.title("Contrôle vocal du drone")
fenetre.geometry("400x400")

main_frame = tk.Frame(fenetre, padx=20, pady=20)
main_frame.pack(expand=True, fill=tk.BOTH)

commands_frame = tk.Frame(main_frame, bd=2, relief=tk.RIDGE, padx=10, pady=10)
commands_frame.pack(fill=tk.X, pady=(0, 10))

tk.Label(commands_frame, text="Commandes Vocales", font=('Arial', 14, 'bold')).pack(pady=(0, 10))

tk.Label(commands_frame, text="• avancer → Vert\n• reculer → Rouge\n• gauche → Bleu\n• droite → Jaune\n• monter → Lavande\n• descendre → Gris\n• atterrissage → Saumon", justify=tk.LEFT).pack()

status_label = tk.Label(main_frame, text="En attente de commande...", font=('Arial', 12), height=2)
status_label.pack(fill=tk.X, pady=(0, 10))

btn_emergency = tk.Button(main_frame, text="ATTERRISSAGE MANUEL", command=lambda: execute_command("atterrissage"), bg='red', fg='white', font=('Arial', 12, 'bold'))
btn_emergency.pack(fill=tk.X)
